...
监控 A 股跨境 ETF 的折溢价率，超出阈值时通过 **Server酱** 推送到微信。
...
| 标的 | 第一档 ⚠️ | 第二档 🚨 | 代码 |
|---|---|---|---|
| 513800 日本东证指数ETF(南方) | \|折溢价\| ≥ **1%** | \|折溢价\| ≥ **2%** | `sh513800` |
| 159866 日经ETF(工银) | \|折溢价\| ≥ **3%** | \|折溢价\| ≥ **5%** | `sz159866` |

规则说明：

- **方向分离**：溢价(>0) 和折价(<0) 分开计算，同一标的、同一方向、同一档位**每天只推一次**
- **回落缓冲**：已在档内时，要跌回「阈值 − 0.5%」才复位；复位后再次突破会重新提醒。
  没有这个缓冲，价格在阈值线上反复蹭就会刷屏
- **高档优先**：多个档位同时被突破时，优先推第二档
- **每日推送总数上限 4 条**（Server酱免费版每天 5 条，留 1 条余量给其他用途）。
  两只标的共享这个额度，谁先触发谁先推
...
.github/workflows/monitor.yml   定时监控工作流（每 5 分钟）
...
5 分钟一次的频率会吃掉大半）→ 不勾选任何初始化选项 → `Create repository`。

**2. 上传文件**
把本目录**全部内容**（含隐藏的 `.github` 目录）传上去。两种方式：
...
**5. 启用并试跑**
`Actions` 标签页 → 左侧选「折溢价监控」→ 如有 `Enable workflow` 就点一下 →
...
```python
TARGETS = (
    {"code": "sh513800", "label": "513800 日本东证指数ETF(南方)", "levels": (1.0, 2.0)},
    {"code": "sz159866", "label": "159866 日经ETF(工银)",         "levels": (3.0, 5.0)},
)

HYSTERESIS = 0.5        # 回落缓冲(百分点)
MAX_PUSH_PER_DAY = 4    # 每日推送总条数上限
```

- `levels` 可给任意档位（如 `(1.0, 2.0, 3.0)` 三档），标识自动生成
- 加标的只需往 `TARGETS` 里追加一条，沪市用 `sh`、深市用 `sz` 前缀

## 已知限制

- **定时有延迟**：GitHub 的 schedule 在高峰期可能延迟数分钟到十几分钟，
  最小粒度为 5 分钟。看折溢价趋势够用，不适合高频交易。
- **60 天限制**：GitHub 会停用连续 60 天没有仓库活动的定时任务。
  仓库里的 `keepalive.yml` 每月 1 号自动打卡一次，正常情况下不会触发。
- **推送额度是共享的**：两只标的一共只有 4 条/天，极端行情下可能被先用完，
